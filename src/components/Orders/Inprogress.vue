...
const totalPages = computed(() => {
    const filteredData = props.data.filter(transaction => transaction.status === 'In Progress');
        ...
        <div class="flex flex-col gap-2">
            <div v-for="data in paginatedData" class="bg-white p-6 rounded border border-gray-200"
                ...
                <div class="flex justify-end border-t py-4">
                    <div class="flex flex-row-reverse lg:flex-row gap-2 w-full lg:w-auto">
                        <f7-button @click="HandleProceed(data.transaction_id, 'Cancelled')"
                            class="refresh-data secondary-button" fill>Cancel</f7-button>
                        <!-- For Vendor -->
                        <f7-button @click="HandleProceed(data.transaction_id, 'Requirements')"
                            v-if="data.vendor_id == authStore.user?.id" class="refresh-data primary-button"
