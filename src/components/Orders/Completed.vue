...
const totalPages = computed(() => {
    const filteredData = props.data.filter(transaction => transaction.status === 'Completed');
            ...
            <div v-for="data in paginatedData" class="bg-white p-6 rounded border border-gray-200">
                ...
                <div class="flex justify-end border-t py-4">
                    <div class="flex w-full md:w-auto">
                        <f7-button @click="HandleProceed(data.transaction_id, 'To Review')" v-if="data.vendor_id != authStore.user?.id" class="primary-button refresh-data" fill>Proceed to review</f7-button>
